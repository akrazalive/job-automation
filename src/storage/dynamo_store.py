...
class DynamoStore(ApplicationStore):
    ...
    def _scan_all(self) -> list[dict]:
        """Loops DynamoDB's OWN internal pagination (its 1MB-per-response
        cap) until exhausted, so results are never silently incomplete
        once the table does grow past that. Fetches the whole table into
        memory every call, filtered in Python afterward via the shared
        src.storage.base.filter_records() (same logic LocalJsonStore
        uses, deliberately kept identical rather than half in a DynamoDB
        FilterExpression and half in Python — a Scan's RCU cost is driven
        by items *read*, not items *returned*, so pushing filters into
        FilterExpression saves network bytes but not read cost; at hobby
        scale that's not worth duplicated, harder-to-keep-in-sync
        filtering logic, and Python-side filtering gets free
        case-insensitive substring matching DynamoDB's own `contains`
        doesn't do natively). If volume grows enough that this gets slow/
    ...
    def list_applications(
        ...
        # as LocalJsonStore — see ApplicationStore.list_applications's
        # CURSOR CONTRACT note), not DynamoDB's own LastEvaluatedKey —
        ...
        items = filter_records(
            self._scan_all(), status=status, source=source, company=company, category=category,
